...
impl<T: Decodable + SignedTransaction> SealedBlock<alloy_consensus::Block<T>> {
    ...
    ///
    /// This leverages [`alloy_consensus::Block::decode_sealed`] which is only available for
    /// blocks with `alloy_consensus::Header`.
    pub fn decode_sealed(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
        let sealed = alloy_consensus::Block::<T>::decode_sealed(buf)?;
        Ok(Self::from(sealed))
    }
}
